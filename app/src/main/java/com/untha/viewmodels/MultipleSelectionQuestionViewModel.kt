...
class MultipleSelectionQuestionViewModel(
    ...
    fun getFaultForQuestion(isNoneOfAbove: Boolean, isLabourRoute:Boolean,
    ...
    private fun processLabourRouteAnswer(isNoneOfAbove: Boolean){
        return if (isNoneOfAbove) {
            saveAnswerOption(question?.options?.last()?.result ?: "", Constants.FAULT_ANSWER_ROUTE_LABOUR)
        } else {
            saveAnswerOption(question?.options?.first()?.result ?: "", Constants.FAULT_ANSWER_ROUTE_LABOUR)
        }
    ...
    private fun processLabourRouteAnswer(options: MutableList<MultipleSelectionOption>){
        when {
            options.filter{ it.isSelected  && it.code.equals("BAJO")}.any() -> {
                saveAnswerOption( "BAJO" , Constants.FAULT_ANSWER_ROUTE_VIOLENCE)
            }
            options.filter{ it.isSelected  && it.code.equals("MEDIO")}.any()->{
                saveAnswerOption( "MEDIO" , Constants.FAULT_ANSWER_ROUTE_VIOLENCE)
            }
            options.filter{ it.isSelected  && it.code.equals("ALTO")}.any()->{
